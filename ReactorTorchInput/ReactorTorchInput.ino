...
#include <TinyPICO.h>
TinyPICO myTP;
unsigned int validFrameCnt = 0;
unsigned int prevSecForFPS = 0;
...
void setup()
{
  myTP = TinyPICO();
  validFrameCnt = 0;
  prevSecForFPS = millis()/1000;

  Serial.begin(115200); // Start the serial console
  //Serial.begin(1000000); // Start the serial console
...
void loop()
{
  ...
  if (myICM.status != ICM_20948_Stat_Ok && myICM.status != ICM_20948_Stat_FIFOMoreDataAvail) // If more data is available then we should read it right away - and not delay
  {
    return;
    delay(1);
  }
...
//      Serial.println(yaw, 1);
      ...
      validFrameCnt++;     
      unsigned int newSecForFPS = millis()/1000;
      if (prevSecForFPS != newSecForFPS)
      {
        //Serial.println("\t\t\t %6ld", validFrameCnt);                
        validFrameCnt = 0;
        prevSecForFPS = newSecForFPS;
      }      
    }
  }
  ...
  float hue = (float) 160;
  float sat = (float) 255;
  int brightnessWave = map(sin16(millis()*64.0 * 10.0), -32767, 32767, 0, 200);
  float brightness = (float) brightnessWave > 100 ? brightnessWave : 0;
  byte r,g,b;  
  HSV_to_RGB(hue, sat, brightness, &r, &g, &b);  
  
  myTP.DotStar_SetPixelColor(r,g,b);
  //myTP.DotStar_CycleColor(10);
}
